...
const captureData = (action, respSet) => {
  ...
  for (const captureItem of action.capture) {
    ...
    let foundData = null;
    foundData = extractData(
      data,
      captureItem.type,
      captureItem.extractionModifiers || { source: captureItem.source }
    );
    ...
    if (captureItem.captureType === "set") {
      if (captureItem.values) {
        ...
        for (const itemValue of captureItem.values) {
          objectWithProperties[itemValue.key] = extractData(
            data,
            itemValue.type,
            itemValue.extractionModifiers
          );
        }
      }
      ...
      if (action.type === "clone-file") {
        return textEditor.getFileText(
          ...
          (err, _parsedText) => {
            ...
            return fs.writeFile(
              `${process.cwd()}/${action.values.outputFileLocation}`,
              _parsedText,
              () => {
                return actionPromise();
              }
      ...
      if (action.type === "update-file") {
        return new Promise((resolve2) => {
          ...
          if (
            ...
            showErr(
              `Exiting this step early, as the file does not exist at location : \r\n ${process.cwd()}/${
                action.values.fileLocation
              }`
            );
          }
...
const runOperation = (_operation, callback, _runCount, iterationOptions) => {
  ...
  if (iterationOptions.type && iterationOptions.iteratorObjects) {
    ...
    if (iterationOptions.type === "for-each-file") {
      ...
      storedValues[iterationOptions.iteratee] =
        iterationOptions.iteratorObjects[runCount].filePath;
      storedValues.iteratedDir =
  ...
  setTimeout(function () {
    ...
    return new Promise((iterationFinished) => {
      ...
      setTimeout(function () {
        ...
        callback();
      }, smartTime(operation.post_delay_loop) || 0);
    });
  }, smartTime(operation.pre_delay_loop) || timeout || 0);
};
